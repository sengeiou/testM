 ...
 * describe:
 * https://www.cnblogs.com/dongweiq/p/7458207.web
 *
 * @sample
 ...
 */
object WebViewUtil {
    fun initWeb(context: Context, webView: WebView, isCache: Boolean) {
        initWebview(webView, context)
        initWebSettings(context, webView, isCache)
    }
    ...
    private fun initWebSettings(context: Context, webView: WebView, isCache: Boolean): WebSettings {
        ...
        if (isCache) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                ...
                webSettings.cacheMode = if (context.isConnected()) WebSettings.LOAD_DEFAULT else WebSettings.LOAD_CACHE_ELSE_NETWORK//没网优先加载缓存数据
            }
...
//        webSettings.setGeolocationDatabasePath(context.filesDir.path)
    ...
    fun loadData(webView: WebView?, content: String) {
        webView?.loadDataWithBaseURL(null, content, "text/web", "UTF-8", null)//这种写法可以正确解码
    }
    ...
    fun removeWeb(webView: WebView) {
        ...
        webView.loadDataWithBaseURL(null, "", "text/web", "utf-8", null)
