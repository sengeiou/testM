...
public class IMMessageManager extends IMManager {
    ...
    public void sendMessage(MessageEntity msgEntity) {
        ULogToDevice.d("sendMessage", TAG, msgEntity.toString());
        logger.d("chat#sendMessage 发送消息, msg:%s", msgEntity.toString());
        // 发送情况下 msg_id 都是0
        // 服务端是从1开始计数的
    ...
    public void onRecvMessage(IMMessage.IMMsgData imMsgData) {
        ...
        String content = recvMessage.getContent();

        logger.d("chat#sendMessage, msg:%s", recvMessage.toString());
    ...
    //透传消息
    public void sendCMD(TextMessage textMessage) {
        if (textMessage == null) {
            return;
        }
        logger.i("chat#text#textMessage");
     ...
     * @param msgList
     */
    public void sendVideos(List<VideoMessage> msgList) {
        logger.i("chat#image#sendVideos size:%d", msgList.size());
        if (null == msgList || msgList.size() <= 0) {
            return;
        }
    ...
    public List<MessageEntity> loadHistoryMsg(int pullTimes, String sessionKey, PeerEntity peerEntity) {
        if (peerEntity == null) {
            logger.d("#loadHistoryMsg# peerEntity is null");
        ...
        List<MessageEntity> msgList = doLoadHistoryMsg(pullTimes, peerEntity.getPeerId(), peerEntity.getType(), sessionKey, lastMsgId, lastCreateTime, count);
    ...
    public List<MessageEntity> loadHistoryMsg(MessageEntity entity, int pullTimes) {
        ...
        List<MessageEntity> msgList = doLoadHistoryMsg(pullTimes, entity.getPeerId(entity.isSend(loginId)), entity.getSessionType(), chatKey, reqLastMsgId, reqLastCreateTime, cnt);
        return msgList;
    }
    ...
    private void onImageLoadSuccess(MessageEvent imageEvent) {
        ...
        imageMessage.setContent(imageMessage.getContent());
    ...
    private void onVideoLoadSuccess(MessageEvent videoEvent) {
        ...
        logger.d("pic#videoUrl:%s", videoUrl);
        String realImageURL = "";
        try {
            realImageURL = URLDecoder.decode(videoUrl, "utf-8");
            logger.d("pic#realImageUrl:%s", realImageURL);
        } catch (UnsupportedEncodingException e) {
            logger.e(e.toString());
        }
        ...
        /**通知Activity层 成功 ， 事件通知*/
        videoEvent.setEvent(MessageEvent.Event.HANDLER_VIDEO_UPLOAD_SUCCESS);
        videoEvent.setMessageEntity(videoMessage);
        triggerEvent(videoEvent);
        videoMessage.setContent(videoMessage.getContent());
