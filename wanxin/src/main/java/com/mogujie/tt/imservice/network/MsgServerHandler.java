...
public class MsgServerHandler extends SimpleChannelHandler {
    ...
    throws Exception {
        ...
        if (null != channelBuffer) {
            ...
            //            while (channelBuffer.readableBytes() >= 5) { // 五位读取
            //                ChannelBuffer tempBuffer = channelBuffer.readBytes(5);
            //                logger.d("channel#messageReceived  read"+tempBuffer.toString(Charset.defaultCharset()));
            //            }
            // 读取剩下的信息
            logger.d("channel#messageReceived read" + channelBuffer.toString(Charset.defaultCharset()));
        }
    }
