...
import com.mogujie.tt.ui.widget.message.RevokeRenderView;
...
public class MessageAdapter extends BaseAdapter {
    ...
    private int mDefaultTimeDifference = 120;   //默认时间差值
    ...
    public void addItem(final MessageEntity msg) {
        ...
        if (msg instanceof ImageMessage) {
            ImageMessage.Companion.addToImageMessageList((ImageMessage) msg);
        }
     ...
     * ----------------------删除一条消息----------------------
     */
    public void removeMsg(MessageEntity messageEntity) {
        //根据消息Id删除数据库内容
        DBInterface.instance().deleteMessageByMsgId(messageEntity.getMsgId());
     ...
     * ----------------------撤回一条消息----------------------
     */
    public void revokeMsg(MessageEntity entity) {
        entity.setDisplayType(DBConstant.SHOW_REVOKE_TYPE);
        //根据消息修改数据库内容
     ...
     * ----------------------修改新收到的消息----------------------
     */
    public void updateRevokeMsg(MessageEntity entity) {
        TextMessage txtMsg = (TextMessage) entity;
        for (Object objectList : msgObjectList) {
            if (objectList instanceof MessageEntity) {
                MessageEntity messageEntity = (MessageEntity) objectList;
                if (messageEntity.getMsgId() == txtMsg.getAttributeInt(MessageExtConst.MSGID)) {
                    ((MessageEntity) objectList).setDisplayType(DBConstant.SHOW_REVOKE_TYPE);
                    //根据消息修改数据库内容
    ...
    private boolean isMsgGif(MessageEntity msg) {
        ...
        return Emoparser.getInstance(this.ctx).isMessageGif(msg.getInfo());
    }
     ...
     * @return
     */
    private UserEntity getUserEntity(MessageEntity textMessage) {
        UserEntity userEntity;
        if (textMessage.getFromId() == loginUser.getPeerId()) {//自己
    ...
    private void getImageList() {
        for (int i = msgObjectList.size() - 1; i >= 0; --i) {
            ...
            if (item instanceof ImageMessage) {
                ImageMessage.Companion.addToImageMessageList((ImageMessage) item);
            }
        }
    }
        ...
        for (int index = len - 1; index > 0; index--) {
            ...
            if (object instanceof MessageEntity) {
                ...
                if (object instanceof ImageMessage) {
                    ImageMessage.Companion.addToImageMessageList((ImageMessage) object);
                }
            ...
            if (obj instanceof Integer) {
                ...
                switch (info.getDisplayType()) {
                    ...
                    case DBConstant.SHOW_ORIGIN_TEXT_TYPE:
                        TextMessage txtMsg = (TextMessage) info;
                        if (info.isGIfEmo()) {
                            type = isMine ? RenderType.MESSAGE_TYPE_MINE_GIF
                                : RenderType.MESSAGE_TYPE_OTHER_GIF;
                        } else if (txtMsg.getContentEntity().getInfoType() == DBConstant.SHOW_REVOKE_TYPE) {
                            type = isMine ? RenderType.MESSAGE_TYPE_MINE_REVOKE
                    ...
                    case DBConstant.SHOW_VIDEO_TYPE:
                        type = isMine ? RenderType.MESSAGE_TYPE_MINE_VIDEO
                            : RenderType.MESSAGE_TYPE_OTHER_VIDEO;
                        break;
                    case DBConstant.SHOW_REVOKE_TYPE:   //撤回
                        type = isMine ? RenderType.MESSAGE_TYPE_MINE_REVOKE
                            : RenderType.MESSAGE_TYPE_OTHER_REVOKE;
                        break;
                    default:
     ...
     * 撤回气泡的渲染展示
     */
    private View revokeBubbleRender(int position, View convertView, ViewGroup parent, final boolean isMine) {
        RevokeRenderView revokeRenderView;
        if (null == convertView) {
            revokeRenderView = RevokeRenderView.inflater(ctx, parent);
        } else {
            // 不用再使用tag 标签了
            revokeRenderView = (RevokeRenderView) convertView;
        }
        if (isMine) {
            revokeRenderView.setContent("您撤回了一条消息");
        } else {
            revokeRenderView.setContent("对方撤回了一条消息");
        }
    ...
    private View imageMsgRender(final int position, View convertView, final ViewGroup parent, final boolean isMine) {
        ...
        UserEntity userEntity = getUserEntity(imageMessage);
        if (null == convertView) {
        ...
        imageRenderView.getMessageFailed().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                // 重发或者重新加载
                MessageOperatePopup popup = getPopMenu(parent, new OperateItemClickListener(imageMessage, position));
                popup.show(messageLayout, DBConstant.SHOW_IMAGE_TYPE, true, isMine, false, position);
            }
        });
    ...
    private View audioMsgRender(final int position, View convertView, final ViewGroup parent, final boolean isMine) {
        ...
        UserEntity userEntity = getUserEntity(audioMessage);
        if (null == convertView) {
        ...
        if (!TextUtils.isEmpty(audioPath)) {
            ...
            messageLayout.setOnLongClickListener(new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    ...
                    //消息是否在2分钟之内创建的
                    boolean bRevoke = (getTime() - audioMessage.getCreated() < mDefaultTimeDifference);
                    popup.show(messageLayout, DBConstant.SHOW_AUDIO_TYPE, bResend, isMine, bRevoke, position);
                    return true;
                }
            });
        }
        ...
        audioRenderView.getMessageFailed().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                MessageOperatePopup popup = getPopMenu(parent, new OperateItemClickListener(audioMessage, position));
                popup.show(messageLayout, DBConstant.SHOW_AUDIO_TYPE, true, isMine, false, position);
            }
        });
    ...
    private View textMsgRender(final int position, View convertView, final ViewGroup viewGroup, final boolean isMine) {
        ...
        UserEntity userEntity = getUserEntity(textMessage);
        if (null == convertView) {
        ...
        textRenderView.getMessageFailed().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                popup.show(textView, DBConstant.SHOW_ORIGIN_TEXT_TYPE, true, isMine, false, position);
            }
        });
        ...
        textView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                //消息是否在2分钟之内创建的
                boolean bRevoke = (getTime() - textMessage.getCreated() < mDefaultTimeDifference);
                popup.show(textView, DBConstant.SHOW_ORIGIN_TEXT_TYPE, bResend, isMine, bRevoke, position);
                return true;
            }
        });

        // url 路径可以设定 跳转哦哦
        final String content = textMessage.getInfo();
        ...
        textRenderView.render(textMessage, userEntity, ctx);// TODO: 2017/11/9 userEntity
    ...
    private View gifMsgRender(final int position, View convertView, final ViewGroup viewGroup, final boolean isMine) {
        ...
        gifRenderView.getMessageFailed().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                MessageOperatePopup popup = getPopMenu(viewGroup, new OperateItemClickListener(textMessage, position));
                popup.show(imageView, DBConstant.SHOW_GIF_TYPE, true, isMine, false, position);
            }
        });
        ...
        imageView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                //消息是否在2分钟之内创建的
                boolean bRevoke = (getTime() - textMessage.getCreated() < mDefaultTimeDifference);
                popup.show(imageView, DBConstant.SHOW_GIF_TYPE, bResend, isMine, bRevoke, position);
    ...
    private View videoMsgRender(final int position, View convertView, final ViewGroup viewGroup, final boolean isMine) {
        VideoRenderView videoRenderView;
        final VideoMessage videoMessage = (VideoMessage) msgObjectList.get(position);
        UserEntity userEntity = getUserEntity(videoMessage);
        if (null == convertView) {
            videoRenderView = VideoRenderView.Companion.inflater(ctx, viewGroup, isMine); //new TextRenderView(ctx,viewGroup,isMine);
        } else {
            videoRenderView = (VideoRenderView) convertView;
        }

        final ImageView videoView = videoRenderView.getMessageContent();
        // 失败事件添加
        videoRenderView.getMessageFailed().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View arg0) {
                MessageOperatePopup popup = getPopMenu(viewGroup, new OperateItemClickListener(videoMessage, position));
                popup.show(videoView, DBConstant.SHOW_GIF_TYPE, true, isMine, false, position);
            }
        });
        ...
        videoView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                MessageOperatePopup popup = getPopMenu(viewGroup, new OperateItemClickListener(videoMessage, position));
                boolean bResend = videoMessage.getStatus() == MessageConstant.MSG_FAILURE;
                //消息是否在2分钟之内创建的
                boolean bRevoke = (getTime() - videoMessage.getCreated() < mDefaultTimeDifference);
                popup.show(videoView, DBConstant.SHOW_GIF_TYPE, bResend, isMine, bRevoke, position);
                return true;
            }
        });
        ...
        videoView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                logger.d("videoMsgRender OnClick url:" + url);
        ...
        videoRenderView.render(videoMessage, userEntity, ctx);
        ...
        try {
            ...
            switch (renderType) {
                ...
                case MESSAGE_TYPE_MINE_VIDEO:
                    convertView = videoMsgRender(position, convertView, parent, true);
                    break;
                case MESSAGE_TYPE_OTHER_VIDEO:
                    convertView = videoMsgRender(position, convertView, parent, false);
                    break;
                case MESSAGE_TYPE_MINE_REVOKE:  //自己撤回
                    convertView = revokeBubbleRender(position, convertView, parent, true);
                    break;
                case MESSAGE_TYPE_OTHER_REVOKE: //別人撤回
                    convertView = revokeBubbleRender(position, convertView, parent, false);
                    break;
            }
            return convertView;
        } catch (Exception e) {
