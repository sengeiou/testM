...
public class AudioRecordHandler implements Runnable {
    ...
    private AudioRecordOutCallBack audioRecordOutCallBack;
    private AudioRecordSoundCallBack audioRecordSoundCallBack;

    public interface AudioRecordOutCallBack {
        void audioRecordOut();
    }

    public interface AudioRecordSoundCallBack {
        void audioRecordSound(int sound);
    }

    public AudioRecordHandler(String fileName, AudioRecordOutCallBack audioRecordOutCallBack, AudioRecordSoundCallBack audioRecordSoundCallBack) {
        super();
        this.fileName = fileName;
        this.audioRecordOutCallBack = audioRecordOutCallBack;
        this.audioRecordSoundCallBack = audioRecordSoundCallBack;
    }
            ...
            try {
                ...
                while (this.isRecording) {
                    ...
                    if (recordTime >= SysConstant.MAX_SOUND_RECORD_TIME) {
                        if (audioRecordOutCallBack != null) {
                            audioRecordOutCallBack.audioRecordOut();
                        }
                        break;
                    }
    ...
    private void setMaxVolume(short[] buffer, int readLen) {
        try {
            ...
            //            Msg.obj = max;
            //            MessageActivity.Companion.getUiHandler().sendMessage(Msg);
            if (audioRecordSoundCallBack != null) {
                audioRecordSoundCallBack.audioRecordSound(max);
            }
        } catch (Exception e) {
            logger.e(e.getMessage());
        }
