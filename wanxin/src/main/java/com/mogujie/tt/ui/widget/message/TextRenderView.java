...
public class TextRenderView extends  BaseMsgRenderView {
    ...
    public void render(MessageEntity messageEntity, UserEntity userEntity,Context context) {
        ...
        // 按钮的长按也是上层设定的
        // url 路径可以设定 跳转哦哦
        String content = textMessage.getInfo();
        CharSequence charContent="";
        if(!TextUtils.isEmpty(content)) {
            charContent = Emoparser.getInstance(getContext()).emoCharsequence(content);
        }
        messageContent.setText(charContent); // 所以上层还是处理好之后再给我 Emoparser 处理之后的
