            ...
            public void onClick(DialogInterface dialog, int which) {
                switch (which) {
                    case 0:
                        ...
                        break;
                    case 1:
                        IMUIHelper.openChatActivity2(ctx, contact);
                        break;
                }
            }
        });
...
//                return R.string.login_error_unexpected;
    ...
    public static void openChatActivity2(Context ctx,PeerEntity peerEntityTo) {
        Intent intent = new Intent(ctx, MessageActivity.class);
        intent.putExtra(IntentConstant.KEY_SESSION_KEY, peerEntityTo.getSessionKey());
        intent.putExtra(IntentConstant.KEY_PEERENTITY, (Serializable) peerEntityTo);
        ctx.startActivity(intent);
    }
    //跳转到用户信息页面
